...
import TokenSelectionOld from '../components/common/TokenSelectionOld';
...
const tokens = [
  ...
  { name: 'USDC', icon: '/icons/tokens/ousd.svg' },
...
const Swap: NextPage = () => {
  ...
  const trade: InterfaceTrade = {
    ...
    inputAmount: { value: fromAmount, currency: fromToken.name },
    outputAmount: { value: toAmount, currency: toToken.name },
    path: [fromToken.name, toToken.name],
  ...
  const calculateToAmount = (amount: string): string => {
    ...
    const CONVERSION_RATE = 0.0899;
    return amount ? (parseFloat(amount) * CONVERSION_RATE).toFixed(6) : '0.0';
  };
