...
const OverViewBox: NextPage<OverviewProps> = ({ assetToBase, selected, maxVal }) => {
  ...
  const returnedTokens =
    amount != undefined && poolUser && Number(amount) > poolUser.getLiabilitiesFloat(reserve)
      ? Number(amount) - poolUser.getLiabilitiesFloat(reserve)
      : 0;

  const amountToReturn = `${toBalance(returnedTokens)} ${selected}`;
  const gasFee = `${toBalance(BigInt((simResponse as any)?.minResourceFee ?? 0), decimals)} XLM`;
  const totalBorrowed = `${toBalance(newPoolUser?.getLiabilitiesFloat(reserve))} ${selected}`;
    ...
    <>
      ...
      <div className="flex items-center justify-center  bg-gradient-to-t to-[rgba(0,0,0,0.1024)] from-[rgba(226,226,226,0.06)] py-4 px-6 font-light">
        <div className="w-3/5 flex flex-col gap-1 text-sm">
          ...
          <Item label="Amount to repay:" value={amount ? amount + ' ' + selected : 0} />
          {selected === 'XLM' && <Item label="Amount to return:" value={amountToReturn} />}
          <div className="flex justify-between">
            <div>
          ...
          <Item label="Your total borrowed:" value={totalBorrowed} />
